...
class GenerativeAlignment(GeneralRecommender):
    ...
    def preprocess_test_missing_modal(self, missing_rate):
        ...
        missing_type = np.random.randint(0, 3, size=n_missing)
        ...
        visual_mask[missing_items[missing_type == 2]] = 0
        textual_mask[missing_items[missing_type == 2]] = 0
        ...
        print("Test - V missing: {}, T missing: {}".format(
                (visual_mask == 0).sum(), (textual_mask == 0).sum()))
    ...
    def pre_epoch_processing(self):
        ...
        n_missing = int(len(train_miss_items) * 0.5)
        ...
        train_visual_missing_mask[train_missing_items[missing_type == 2]] = 0
        train_textual_missing_mask[train_missing_items[missing_type == 2]] = 0
        ...
        print("Train - Total available items {} V missing: {}, T missing: {}".format( n_missing, (self.train_visual_missing_mask == 0).sum(), (self.train_textual_missing_mask == 0).sum()))
    ...
    def get_z_star(self, num_mods, means, logvars, missing_mask):
        # num_mods = len(feats)
        batch_size = means[0].shape[0]
        dim = means[0].shape[1]

        inverse_var = torch.zeros(batch_size, dim, device=self.device, dtype=torch.float32)
        inverse_var_weighted_mean = torch.zeros(batch_size, dim, device=self.device, dtype=torch.float32)
        
        for i in range(num_mods):
            mean_i = means[i].to(torch.float32)  # [batch_size, dim]
            var_i = torch.exp(logvars[i]).to(torch.float32)  # [batch_size, dim]
            
            # mask[:, i:i+1] -> [batch_size, 1] 
            m = missing_mask[:, i:i+1]  # [batch_size, 1]
            
            inverse_var += m / var_i
            inverse_var_weighted_mean += m * mean_i / var_i
    ...
    def flow_matching_generation(self, item_id_embeds, gcn_user_embs, image_embeds, text_embeds, train=False):
        ...
        feat_v_gate = self.gate_z(item_id_embeds * v_mean)

        feat_t_gate = self.gate_z(item_id_embeds * t_mean)

        feat_gate = torch.cat((feat_v_gate, feat_t_gate), dim=-1)
        feat_gate = F.softmax(feat_gate, dim=-1)

        items_mean = [v_mean, t_mean]
        items_logvar = [logvars, logvars]


        z_mean, z_var = self.get_z_star(num_mods=len(items_mean),  means=items_mean, logvars=items_logvar, missing_mask=feat_gate)
        ...
        if train:
            image_conv = generated_v_feat
            text_conv  = generated_t_feat
        else:

            image_conv = (original_v_feat + generated_v_feat) / 2
            text_conv  = (original_t_feat + generated_t_feat) / 2

        self.trans_image_embedding.data = image_conv
        self.trans_text_embedding.data = text_conv        

            
        _, id2t_alignment_loss = self.z2v_flow_model.compute_loss(reparameterize_z_feat, original_v_feat.detach(), item_history_user_embeds) 
        _, id2v_alignment_loss = self.z2t_flow_model.compute_loss(reparameterize_z_feat, original_t_feat.detach(), item_history_user_embeds)
        
        
        generation_loss = (id2t_alignment_loss + id2v_alignment_loss) / 2


        return image_conv, text_conv, reparameterize_z_feat, generation_loss
    ...
    def forward(self, adj, train=False):
        ...
        image_conv, text_conv, fusion_conv, generation_loss = self.flow_matching_generation(item_embeds, gcn_user_embs, image_feats, text_feats, train)
        ...
        image_embeds = torch.multiply(image_prefer, image_embeds)
        text_embeds = torch.multiply(text_prefer, text_embeds)
    ...
    def calculate_loss(self, interaction):
        ...
        Vae_loss = vae_v_loss + vae_t_loss
