...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        self.eval_i = self.dataset.df[self.dataset.iid_field].unique()
        # self.all_item_num = len(self.additional_dataset.df[self.dataset.iid_field].max())
        # self.all_item_num = self.additional_dataset.df[self.dataset.iid_field].max() + 1
    ...
    def _next_batch_data(self):
        ...
        return [batch_users, batch_mask_matrix, self.eval_i]
